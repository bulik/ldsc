...
class __GenotypeArrayInMemory__(object):
    ...
    def __corSumVarBlocks__(self, block_left, c, func, snp_getter, annot=None, covar=None):
        '''
        Parameters
        ----------
        ...
        annot : numpy array with shape (m,n_a)
            SNP annotations.
        covar : np.ndarray with shape (n, k)
            Covariates.
        ...
        if covar is not None:
            Pcovar = covar.dot(np.linalg.pinv(covar.T.dot(covar)))
