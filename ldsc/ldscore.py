...
class __GenotypeArrayInMemory__(object):
	...
	def __corSumBlockJackknife__(self, block_left, c, func1, snp_getter, annot=None, jN=10,
		func2=np.dot):
		'''
		Computes block jackknife estimate and standard error.
		
		Parameters
		----------
		block_left : np.ndarray with shape (M, ) and dtype int.
			block_left[i] = index of leftmost SNP included in LD Score of SNP i.
			if c > 1, then only entries that are multiples of c are examined, and it is 
			assumed that block_left[a*c+i] = block_left[a*c], except at 
			the beginning of the chromosome where the 0th SNP is included in the window
		c : int
			Chunk size.
		func : function
			func should take arguments (R, n, annot) and return an np.ndarray with shape equal
			to annot.shape.
		snp_getter : function(int)
			The method to be used to get the next SNPs (normalized genotypes? Normalized 
			genotypes with the minor allele as reference allele? etc)
		...
		Returns
		-------
		cor_sum_jn : np.ndarray with shape (M, num_annots)
			Block jackknife estimates (which remove O(1/N) bias, where N is sample size).
		
		cor_sum_se : np.ndarray with shape (M, num_annots)
			Standard errors of cor_sum (cor_sum_se[i,j] is the standard error of cor_sum[i,j]).
			
		Algorithm
		-----------
		Suppose we wish to apply a delete-k block jackknife to a function f. Let f[-i] denote
		the i^th delete-k value, and let f(i) denote the i^th block value. If there exists a 
		function g such that we can write
		
		f[-i] = g(f(1), ... , f(i-1), f(i+1), ... , f(Nb) ), 
		
		then we can compute an inexpensive block jackknife via the following algorithm:
		
		(1) compute Nb block values f(1), ..., f(Nb)
		(2) compute Nb delete-k values by applying function g to the block values
		
		For our purposes, functions g and f can be computed in linear time. In this case, the 
		complexity of the naive block jackknife algorithm (computing each delete-k value 
		directly from the data) is O(Nb*(N-k)). For the improved algorithm, we get O(k*Nb) 
		for computing the block values then Nb^2 for computing the delete-k values from the 
		block values, which comes out to O(Nb*(k+Nb)). Since k+Nb << N-k (recall that N=k*Nb),
		the improved algorithm is a win. Note that we can do better if g is a linear function 
		(linear in the sense of algebra, not computational complexity). In this case, the 
		jackknife estimator and standard error can be written directly in terms of the block
		values, so it is not necessary to compute the delete-k values, so we can perform the
		delete-k jackknife in O(Nb*k) time.
		
		In practice, f(i) is a Pearson correlation coefficient between genotypes at two SNPs
		computed from individuals in the i^th block. The i^th delete-k value of the Pearson 
		correlation coefficient between the genotypes at these two SNPs is simply the mean
		of f(1),..., f(i-1), f(i+1),...,f(Nb). Let r[i]_jk denote this delete-k value. If we
		wish to apply a delete-k block jackknife to a function f such that 
		
		f[i] = g(r[i]_jk), for some function g,
		
		then we can accomplish this by passing g as the func1 argument to this method. The
		optional func2 argument allows slightly more flexibility since we might be computing 
		several block jackknives simultaneously (e.g., in the case of partitioned LD Score
		estimation).
...
class VcfBINFile(__GenotypeArrayInMemory__):
	...
	def __read__(self, fname, m, n):
		...
		n, m = self.n, self.m
		c = "Reading genotypes for {n} individuals and {m} SNPs from {b}"
		#print c.format(n=n,m=m,b=fname)
