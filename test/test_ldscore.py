...
class test_snp(unittest.TestCase):
	...
	def test_snp(self):
		snp = ld.VcfSNPFile('test/vcf_test/test.snp')
		print snp.IDList
		assert snp.n == 6
	...
	@nose.tools.raises(ValueError)
	def test_bad_filename(self):
		ind = ld.VcfSNPFile('test/plink_test/plink.asdf')
...
class test_ind(unittest.TestCase):
	...
	def test_ind(self):
		ind = ld.VcfINDFile('test/vcf_test/test.ind')
		print ind.IDList
		assert ind.n == 5
	...
	@nose.tools.raises(ValueError)
	def test_bad_filename(self):
		ind = ld.VcfINDFile('test/plink_test/plink.snp')
		
		
class test_fam(unittest.TestCase):

	def test_fam(self):
		fam = ld.PlinkFAMFile('test/plink_test/plink.fam')
		print fam.IDList
		assert fam.n == 5
	...
	@nose.tools.raises(ValueError)
	def test_bad_filename(self):
		fam = ld.PlinkFAMFile('test/plink_test/plink.bim')


class test_bim(unittest.TestCase):

	def test_bim(self):
		bim = ld.PlinkBIMFile('test/plink_test/plink.bim')
		print bim.IDList
		assert bim.n == 8
	...
	@nose.tools.raises(ValueError)
	def test_bad_filename(self):
		bim = ld.PlinkBIMFile('test/plink_test/plink.fam')
	
		
class test_filter_file(unittest.TestCase):

	def test_filter(self):
		filter = ld.FilterFile('test/vcf_test/test.ind')
		print filter.IDList
		assert filter.n == 5
		correct = np.array(['per1', 'per2', 'per3', 'per4', 'per5'])
