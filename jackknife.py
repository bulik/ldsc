...
class Hsq(object):
	...
	def __init__(self, chisq, ref_ld, w_ld, N, M, num_blocks=200, non_negative=False,
		...
		x = _weight(x, np.multiply(weights,np.sqrt(weights)))
		y = _weight(chisq_m_int, np.multiply(weights,np.sqrt(weights)))
		...
		residuals = y - np.dot(x,self._jknife.est.T)
		residuals = np.hstack((residuals, ref_ld))
		np.savetxt('resid_scz',residuals)
