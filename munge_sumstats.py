...
def get_log_header(args, defaults, log):
	opts = vars(args)
...
def munge_sumstats(args, p=True): # set p = False for testing in order to prevent printing
	...
	try:	
		...
		if p:
			log.log(get_log_header(args, vars(parser.parse_args('')), log))
